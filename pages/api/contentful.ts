...
export async function getEntries(type: string, queryParams?: Record<string, string>) {
  const client = createClient({
    accessToken: isDev ? process.env.CONTENTFUL_PREVIEW_TOKEN as string : process.env.CONTENTFUL_DELIVERY_TOKEN as string,
...
function mapEntry(entry: any) {
  return {
    ...
    fields: Object.entries(entry.fields).reduce((acc: Record<string, any>, [key, value]) => {
